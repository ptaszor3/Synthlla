...
public:
	synTremolo(wxWindow* parent, wxWindowID id = wxID_ANY, wxPoint position = wxDefaultPosition, wxSize size = wxDefaultSize)
	:wxWindow{parent, id, position, size}, Tremolo{signal_manager.get_default_signal(), 5, effects::SynchronizationType::Synchronized, 0.3} {
		max_volume_deviation_slider = new synSlider(this, wxID_ANY, "Maximal influence", 0, 100, 30, reinterpret_cast<void*>(&this->maximal_volume_deviation), [](void* data, int value)->void{*reinterpret_cast<double*>(data) = value / 100.0;}, {0, 0}, synSlider::Styles::FIXED);
